...
switch _mode do
{
	case "onLoad":
	{
		...
		// Buttons
		private ["_createLeaveKickButton", "_disbandButton", "_joinButton", "_kickPlayerButton", "_invitePlayerButton", "_closeInterfaceButton", "_tabGroupsButton", "_tabPlayersButton"];
		_createLeaveKickButton 	= _display displayCtrl IDC_DYNAMICGROUPS_BUTTONCREATELEAVEKICK;
		_disbandButton 			= _display displayCtrl IDC_DYNAMICGROUPS_BUTTONPROMOTEDISBAND;
		...
		_disbandButton ctrlAddEventHandler ["ButtonClick", { with uiNamespace do { ["OnDisbandButtonClick", _this] call DISPLAY; }; }];
	...
	case "UpdateManageSection" :
	{
		...
		{
			private ["_groupName", "_groupInstance", "_groupLeader", "_groupPlayers", "_groupSide", "_groupIsPrivate"];
	...
	case "FillGroupsListbox" :
	{
		...
		// Add them to the list box
		{
			...
			private ["_groupName", "_groupIsPrivate", "_groupLeader", "_groupPlayers", "_wasKickedFrom", "_locked", "_maxUnitCount", "_isFull", "_autoName"];
			...
			_locked			= (_groupIsPrivate || {_wasKickedFrom}) && {!_hasInvite};
			...
			{
				private ["_groupPlayerName", "_isLeader", "_sameGroup", "_groupPlayerIcon"];
	...
	case "FillPlayersListbox" :
	{
		...
		{
			_newPlayersList pushBack [_x, [_x] call BIS_fnc_getName, group _x, groupId group _x, leader group _x == _x];
		} forEach _playerList;
		...
		if !(_newPlayersList isEqualTo _oldPlayersList) then
		{
			...
			// Fill listbox
			{
				...
				private ["_name", "_textureIcon", "_textureRank", "_isGroupLeader", "_texture", "_sameGroup"];
	...
	case "OnDisbandButtonClick" :
	{
		...
		if (LOG_ENABLED) then
		{
			["OnDisbandButtonClick: %1", _isSelfSelected] call BIS_fnc_logFormat;
		};
	};
